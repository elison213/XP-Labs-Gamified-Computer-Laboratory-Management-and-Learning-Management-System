...
$stats = $db->fetch(
    "SELECT 
            ...
            AVG(TIMESTAMPDIFF(MINUTE, sa.check_in_time, COALESCE(sa.check_out_time, NOW()))) as avg_duration_min
     FROM station_assignments sa
     WHERE $whereClause",
    $params
);
...
<body>
    ...
    <div class="main-content">
        ...
        <div class="row g-3 mb-4">
            ...
            <div class="col-md-4">
                <div class="stat-card">
                    <div class="value"><?= $stats['avg_duration_min'] ? round($stats['avg_duration_min']) : 0 ?>m</div>
                ...
                <form method="GET" class="row g-2">
                    ...
                    <div class="col-md-2">
                        ...
                        <select name="lab_id" class="form-select">
                            ...
                            <option value="<?= $l['id'] ?>" <?= $labFilter == $l['id'] ? 'selected' : '' ?>><?= e($l['name']) ?></option>
                            <?php endforeach; ?>
                        </select>
                    </div>
                    <?php endif; ?>
                    ...
                    <div class="col-md-2 d-flex align-items-end">
                        <a href="attendance_history.php" class="btn btn-outline-secondary w-100">Clear</a>
                    </div>
                </form>
            </div>
        </div>
                        ...
                        <tbody>
                            ...
                            <tr>
                                <td>
                                    ...
                                    <div class="text-muted small"><?= e($r['lrn']) ?></div>
                                </td>
                                <td>
                                    <?= e($r['lab_name'] ?? $r['floor_name']) ?>
                                    <div class="text-muted small"><?= e($r['station_code']) ?></div>
                                </td>
                                <td><?= date('M j, Y H:i', $checkIn) ?></td>
                                <td><?= $checkOut ? date('M j, Y H:i', $checkOut) : '<span class="text-muted">—</span>' ?></td>
                                <td><?= $durationMin ?> min</td>
                                <td>
                                    <span class="status-dot <?= $isActive ? 'active' : 'completed' ?>"></span>
